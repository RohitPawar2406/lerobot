...
class ConfigClass:
    ...
    run_name: str = "with_xavier_for_decode"
...
class ModelNetwork(nn.Module):
    def __init__(self, cfg):
        ...
        nn.init.xavier_uniform_(new_out.weight) # Initialize Output Projection
        ...
        nn.init.xavier_uniform_(new_in.weight) # Initialize Input Projection
        if new_in.bias is not None:
            nn.init.zeros_(new_in.bias)    
...
# Initialize the progress bar with total steps
    with tqdm(total=cfg.max_steps, desc="Training Steps", unit="step") as pbar:
        while global_step < cfg.max_steps:
            for batch in dataloader:
                ...
                for i in prompts:
                    ...
                    # Update progress bar and show current loss
                    pbar.update(1)
            ...
            # Break the outer loop if max steps reached
            if global_step >= cfg.max_steps:
                break
